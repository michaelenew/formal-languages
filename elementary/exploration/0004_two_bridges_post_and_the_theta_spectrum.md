...
- The packaging lemma (0001 §1) is the EL version of a **Webb-type
  function** (Webb 1935: every finite k-valued logic is generated by one
  binary operation; the specific formula `max(x, y) + 1 mod k` is
  unverified here, see 0005 §4), built from a lattice-like operation and
  a cyclic bijection. EML is built from a group operation
...
becomes a coordinate. Both workstreams meet the same obstruction — a unipotent operator with
no eigenbasis — and cure it the same way, by a lift. The branch cut of
0001 §5 is what remains when the lift is undone by projection.
*Literature check (0005): both halves are standard — Deligne's canonical
extension and the monodromy theorem on one side, modular representation
theory and Brauer characters on the other — so this is a recognised
instance, not a new observation.*
